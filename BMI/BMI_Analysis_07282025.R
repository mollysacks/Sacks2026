...
# plot BMI, age, and sex distributions
...
bmi_hist <- ggplot(genomatrix, aes(x=BMI_raw, color=sex, fill=sex)) +
...
ggsave('AnalysisOutput/raw_bmi_distribution.png', bmi_hist, height = 8, width= 8)
...
for(c in seq_along(CNVs)){
  ...
  sumstats_raw <- genomatrix %>%
    filter(gen == cn) %>%
    summarise(
      mean = mean(BMI_raw, na.rm = TRUE),
      sd = sd(BMI_raw, na.rm = TRUE),
...
# PRS only
model_PRS <- lm(BMI ~ PRS + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
                data = genomatrix, na.action=na.exclude)
...
for(t in seq_along(loci_trios)){
  ...
  model_additive <- lm(BMI ~ locusA + locusB + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
                       data = genomatrix, na.action=na.exclude)
  model_int <- lm(BMI ~ locusA*locusB + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, 
                  data = genomatrix, na.action=na.exclude)
